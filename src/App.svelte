    ...
    try {
      ...
      if (parsed.kind === 'update') {
        ...
        if (store.data.plan) {
          ...
          await dialogs.alert('По этой ссылке — полный план, но у вас уже есть настроенный дашборд. Чтобы ничего не потерять, ничего не заменялось. Если замена действительно нужна — Профиль → «Загрузить из файла».');
          return;
        }
